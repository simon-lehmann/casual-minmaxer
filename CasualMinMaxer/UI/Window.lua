...
UI.SOURCE_TYPES = { "Q", "B", "R", "N", "T", "G", "V", "E", "H", "A", "F", "K", "S", "W" }
UI.SOURCE_NAMES = {
  ...
  K = "Crafted", S = "Auction house (random suffix)", W = "World drop (BoE)",
}
...
function UI.CharDB()
  ...
  if not c.filters then
    ...
    for _, s in ipairs(UI.SOURCE_TYPES) do c.filters.sources[s] = (s ~= "A") end
  end
  c.filters.sources = c.filters.sources or {}
  for _, s in ipairs(UI.SOURCE_TYPES) do
    if c.filters.sources[s] == nil then c.filters.sources[s] = (s ~= "A") end
  end
